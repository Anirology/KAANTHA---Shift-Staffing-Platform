...
export function ManageShifts({ onNavigate }) {
  ...
  return <div className="screen">
    <header className="screen-heading"><div><span className="intro-eyebrow">Business</span><h1>Manage shifts</h1><p>Review your shifts and the workers who applied.</p></div><Button type="button" onClick={() => onNavigate('/business/shifts/new')}>Create shift</Button></header>
    ...
    <DataState loading={loading} error={error} onRetry={retry} empty={shifts.length === 0} emptyMessage="You have no shifts yet. Create your first shift to get started.">
      <div className="card-list">{shifts.map((shift) => <ShiftCard key={shift.id} shift={shift} actions={<>
        ...
        <Button type="button" variant="secondary" onClick={() => onNavigate(`/business/shifts/${shift.id}/applicants`)}>Applicants</Button>
        ...
        <Button type="button" variant="secondary" disabled={processingId !== null || ['CANCELLED', 'COMPLETED'].includes(shift.status)} onClick={() => cancel(shift)}>{processingId === shift.id ? 'Cancelling…' : 'Cancel'}</Button>
      </>} />)}</div>
    </DataState>
  </div>
}
