...
export function Reports() {
  ...
  return <div className="screen reports-screen"><header className="screen-heading"><div><span className="intro-eyebrow">KAANTHA · BUSINESS</span><h1>Business reports</h1><p>Review live shift data and download the same filtered rows as CSV.</p></div><Button type="button" variant="secondary" disabled={loading || exporting || !!error} onClick={download}>{exporting ? 'Preparing…' : 'Download CSV'}</Button></header>
    ...
    <section className="panel report-results"><span className="report-watermark" aria-hidden="true">KAANTHA</span><h2>{reports[kind].title} report</h2><p className="report-caption">KAANTHA business data</p><DataState loading={loading} error={error} onRetry={() => { setLoading(true); load() }} empty={rows.length === 0} emptyMessage="No rows for these dates."><div className="table-scroll"><table><thead><tr>{reports[kind].fields.map((field) => <th key={field}>{label(field)}</th>)}</tr></thead><tbody>{rows.map((row) => <tr key={rowKey(kind, row)}>{reports[kind].fields.map((field) => <td key={field}>{cell(field, row[field])}</td>)}</tr>)}</tbody></table></div></DataState></section>
  </div>
}
